...
if ($sqlaccount > 0) {
    ...
    if ($sqlloanTransaction == '') {
        $InstallmentDate = date('Y-m-d', strtotime($sqlaccount['FirstInstallmentDate']));
    } else {
    ...
    if ($sqlaccount['Balance'] > $emi) {
        ...
        if ($data == 0) {
            ...
            if ($sqlloanTransaction == '') {
                ...
                if ($currentdate > $InstallmentDate) {
                    $days = round(abs(strtotime($start) - strtotime($nextday)) / 86400);
                } else {
    ...
    <?php if ($sqlaccount['Balance'] != '0') { ?>
        ...
        <?php } ?>
            ...
            <input type="text"  name="odintrest" id="odintrest" class="form-control"  readonly >
